...
namespace Moonrise.Utils.Standard.Config
{
    ...
    public class Settings
    {
        ...
        /// </summary>
        public static string EncryptionIdentifier { get; set; } = "[{ENC]{";
        ...
        /// </summary>
        public ISettingsEncryptor SettingsEncryptor { get; set; } = new DpApiSettingsEncryptor(DpApiSettingsEncryptor.ProtectionScope.Machine);
        ...
        public static string Decrypt(string readVal, ISettingsEncryptor settingsEncryptor)
        {
            ...
            while (moreEncryptedStrings)
            {
                try
                {
                    string preEncrypted = readVal.Extract(ref start, $"{EncryptionIdentifier}", false);

                    if (preEncrypted.EndsWith("\""))
                    {
                        preEncrypted = preEncrypted.Remove(preEncrypted.Length - 1);
                    }
                    ...
                    try
                    {
                        base64Str = readVal.Extract(ref start, "\"", false);
                    }
                ...
                {
                    throw new SettingsException(excep, SettingsExceptionReason.UnknownException, nameof(Decrypt), excep.Message);
                }
            }
        ...
        public bool Read<T, U>(string key, T instance, Expression<Func<U>> property, bool addIfNotExists = false)
        {
            ...
            {
                throw new ArgumentException($"The property is null, please assign it before passing by reference!", nameof(property));
            }

            bool retVal = Read(key, ref referableValue, addIfNotExists);
        ...
        /// </exception>
        [SuppressMessage("StyleCop.CSharp.LayoutRules", "SA1503:CurlyBracketsMustNotBeOmitted", Justification = "I excuse throws!")]
        ...
        public bool Read<T>(string key, ref T value, bool addIfnotExists = false)
        {
            ...
            string setting = ReadCommon(key, _type);
            ...
            where U : IConvertible
        {
            ...
            if (referableValue == null)
            {
                throw new ArgumentException($"The {property.Name} property is null, please assign it before passing by reference!", nameof(property));
            }

            bool retVal = ReadEnum(key, ref referableValue, addIfNotExists, what);
            MemberExpression expr = (MemberExpression)property.Body;
            PropertyInfo prop = (PropertyInfo)expr.Member;
            prop.SetValue(instance, referableValue, null);
        ...
        private string Encrypt(string writeThis)
        {
            ...
            byte[] encryptedData = SettingsEncryptor.Encrypt(writeThis);
            string base64Str = Convert.ToBase64String(encryptedData);

            retVal = $"{EncryptionIdentifier}{base64Str}";
            return retVal;
        }
        ...
        {
            string readVal = Provider.ReadSetting(key, type);

            // We need to check if the string is an encrypted string
            if ((readVal != null) && readVal.Contains(EncryptionIdentifier))
            {
                try
                {
