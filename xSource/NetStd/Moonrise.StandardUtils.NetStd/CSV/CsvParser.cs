...
namespace Moonrise.Utils.Standard.CSV
{
        ...
        where TOverlay : class
    {
        ...
        /// <param name="trimData">Should the qualifiers be trimmed?</param>
        /// <returns>An enumerable of imported rows of the Type</returns>
        public IEnumerable<TTarget> Parse(string[] lines, string delimiter = ",", string qualifier = "\"", bool trimData = true)
        {
            TTarget[] targets = new TTarget[lines.Length - 1];
            ...
            Parallel.For(1, lines.Length, i => { ProcessRow(lines[i], targets, i - 1, delimiter, qualifier, trimData); });
        ...
        private void BuildColumnDefinitions()
        {
            ...
            foreach (PropertyInfo overlayProperty in overlayProperties)
            {
                ...
                if (csvAttribute != null)
                {
                    ...
                    {
                        // And it's consequent TypeConverter
            ...
            foreach (ColumnDefinition defn in _columns.Values)
            {
                ...
                catch (Exception e)
                {
                    if (CollateExceptions)
                    {
                        ...
                        CollatedExceptions.Add(new CsvParseException(i, defn.Name, e));
                    }
                    else
                    {
                        throw new CsvParseException(i, defn.Name, e);
                    }
                }
            }
        }
