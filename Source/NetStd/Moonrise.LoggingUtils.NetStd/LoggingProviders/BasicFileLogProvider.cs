...
namespace Moonrise.Logging.LoggingProviders
{
    ...
    public class BasicFileLogProvider : ILoggingProvider, IAuditProvider
    {
        ...
        public enum Cycle
        {
            ...
            ///     <para>The format for the datetime part of the filename can be set via <see cref="Config.ByCountFilenameDateTimeFormat"/>.
            ///     </para>
            /// </summary>
            ByCount
        }
        ...
        /// later ingestion by LogStash/ElasticSearch - then this override will allow that.<para>
        /// NOTE: This method is only called AFTER all of the appropriate logging enablement checks have been performed
        /// so all you need to do is format the message.</para>
        /// </summary>
        ...
        private string ConstructFilename(string filename, DateTime now, string extension)
        {
            ...
            switch (Recycle)
            {
                ...
                case Cycle.ByCount:
                {
                    ...
                    {
                        _count = 0;
                        retVal = string.Format("{0}_{1}{2}{3}", filename, now.ToString(config.ByCountFilenameDateTimeFormat), threadId, extension);
                    }
